...
class WeeklyActivityChart extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final maxHours = activityData.isNotEmpty
        ? activityData
            .map((data) => data['hours'] + (data['minutes'] / 60))
            .reduce((a, b) => a > b ? a : b)
        : 0;

    final double minBarHeight = 20.0; // 최소 막대 높이
    final double maxBarHeight = 150.0; // 최대 막대 높이로 화면 넘침 방지
                ...
                Container(
                  ...
                  decoration: BoxDecoration(
                    color: _getColorForActivity(
                        totalActivityTime), // 활동 시간에 따라 색상 변경
  ...
  Color _getColorForActivity(double hours) {
    if (hours < 1) {
      return Colors.orange.shade200; // 1시간 미만
    } else if (hours >= 1 && hours < 3) {
      return Colors.orange.shade400; // 1시간 이상 3시간 미만
    } else if (hours >= 3 && hours < 5) {
      return Colors.pink.shade300; // 3시간 이상 5시간 미만
    } else {
      return Colors.redAccent; // 5시간 이상
    }
  }
