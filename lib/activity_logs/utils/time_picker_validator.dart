...
class TimePickerValidator {
  ...
  final DateTime? _cachedMinDateTime;
  ...
  TimePickerValidator({
    ...
    DateTime? cachedMinDateTime,
        ...
        _cachedMinDateTime = cachedMinDateTime;

  bool get isBreakType => item['type'] == 'break';

  /// beforeItem에서 최소 허용 시간을 가져옴
  DateTime? getMinDateTime() {
    if (item['type'] == 'activity_start') {
      return _cachedMinDateTime?.add(const Duration(minutes: 1));
    }
  ...
  /// 선택된 시간이 유효한지 검증
  bool isValidDateTime(DateTime selectedTime, bool isEndTime,
    ...
    if (isBreakType) {
      if (isEndTime) {
        ...
        if (selectedTime.isAfter(endTime) ||
            selectedTime.isAtSameMomentAs(endTime)) {
          return false;
        }
      }
    }
  ...
  List<int> getValidHoursForDate(DateTime date, bool isEndTime,
      DateTime Function(bool) getCurrentDateTime) {
    List<int> validHours = [];
  ...
  List<int> getValidHoursIncludingCurrent(
    ...
    List<int> validHours =
        getValidHoursForDate(date, isEndTime, getCurrentDateTime);
  ...
  List<int> getValidMinutesIncludingCurrent(
    ...
    List<int> validMinutes =
        getValidMinutesForDateTime(date, hour, isEndTime, getCurrentDateTime);
  ...
  List<int> getValidMinutes(
    ...
    return getValidMinutesForDateTime(
        currentDate, currentDateTime.hour, isEndTime, getCurrentDateTime);
  }
}
